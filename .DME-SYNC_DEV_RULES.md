...
Before making any changes or beginning new work, every contributor (human or AI) **must**:

1. **Review Project Status:**
   - Read the "Current Status" and "Handoff Checklist" sections in the README.
   - Read the latest `PROJECT_BRIEF.md` for a summary of the project state and next steps.

2. **Audit the Codebase:**
   - Examine the current code structure and key modules.
   - Review recent commit history for context on recent changes and decisions.

3. **Check Run Logs and Manifests:**
   - Review the latest run logs and manifests (e.g., `run_history.jsonl` in S3) to understand the most recent pipeline executions and data state.

4. **Document Assumptions:**
   - Clearly document any new assumptions, discoveries, or context in the relevant issue, PR, or commit message **before** implementation.
...
- Always call `load_dotenv()` at the top of all scripts and test files that require environment variables.
- If environment variables are not visible to Python, ensure they are loaded from `.env` and/or set in the shell before running code or tests.
- If you encounter errors about missing environment variables, print all environment variables at the start of the script/test (e.g., print(os.environ)), confirm `.env` is present, and reload as needed. If the variable is still missing, ensure it is set in your shell or in `.env` in the current working directory. When using Poetry, prefer `poetry run` to ensure the correct environment is used.
- All major dependency upgrades, downgrades, and removals must be documented in DEPENDENCY_CHANGES.md, including rationale and any conflicts encountered. This ensures future maintainers understand the history and reasoning behind dependency changes.
- Pinecone hybrid index and in-cluster embedding require pinecone-client >=3.0, which is not compatible with Python 3.13+. Use Python 3.12.x for hybrid features, or continue with legacy pipeline on 3.13+.
- **Documentation Update Rule:**
  Whenever you change the environment setup, dependencies, or tooling (e.g., Python version, Poetry/venv, install steps), you **must** immediately update the "Getting Started: Environment Setup" section in the README.md (and any other relevant onboarding docs) in the same commit as your code change. This is mandatory for all PRs and direct pushes.
...
# Current Status (update after each major step)
- Completed: Section 2 (Normalize & Canonicalize, verified; code and rules pushed)
- In Progress: Section 4 (Chunk, Embed, Index)
- Next: Section 5 (Retrieval Pipeline)
- Blockers: None

- After any significant bug fix or feature that passes tests, immediately commit and push before proceeding to further integration or new features. Do not batch unrelated changes into a single commit.

- All Python subdirectories (packages) must contain an __init__.py file, even if empty, to ensure proper import resolution and package recognition.

- At the beginning of the first session, the assistant must announce their persona (Fred, expert DevOps architect) and ask for the user's preferred name so all communication can be personalized.

- After any destructive operation (e.g., table truncate, S3 delete, index clear), always verify the data source and path by running a direct query or listing before proceeding. Document the verification step in the run log or commit message. This prevents silent data path errors and ensures auditability.

- After any pipeline stage that produces or transforms data (e.g., scraping, normalization, chunking), immediately verify the output by inspecting a sample of the resulting records for expected content (e.g., non-empty text fields). If the output is empty or malformed, halt and debug before proceeding. Document this verification in the run log or commit message.

- When stating an intention to inspect, verify, or debug, always immediately execute the necessary commands or code to perform the inspection, rather than waiting for further user input. This ensures actions match stated intentions and prevents workflow stalls.

- **Import and Test Execution Rule:**
  All scripts and tests must be run as modules from the project root, with `PYTHONPATH=.` set, to ensure consistent import resolution. This applies to all local, CI, and production runs.

- **Planned Refactor:**
  After a successful end-to-end pipeline test, migrate all code modules to a `src/` layout for improved import hygiene and maintainability. Update all run and test instructions accordingly.

- **Automated Data Parity Audits:**
  Automated end-to-end audit scripts must be maintained and run after every major pipeline stage and before production deployment. These scripts must verify parity between S3 manifests, Postgres normalized records, and Pinecone (or vector DB) upserts, reporting any discrepancies. Audit results should be logged and reviewed as part of the CI/CD process.

- All tests must use real data and exercise the full pipeline. Mocking is only permitted for error-path or destructive operation simulation, and must be clearly documented.

- Long-running or resource-intensive commands (e.g., full pipeline runs, re-indexing, large data operations) should be executed in a background process or separate terminal, allowing development and collaboration to continue uninterrupted. Always notify the team or user when such a process is started and when it completes.

- All code or pipeline changes must be accompanied by immediate updates to all relevant documentation (README, dev rules, backlog, etc.) in the same commit. Never push code without updating documentation to reflect the current state.

**Endpoint Sanity Check Rule:**
Before running or debugging any full test suite, always perform a direct, minimal endpoint test (e.g., upsert/query for vector DBs, DB connection, API ping) to verify:
  1. The endpoint is reachable and healthy.
  2. The environment variables (API keys, index names, etc.) are correct and visible to the test process.
  3. The test data is isolated and the environment is clean (e.g., test index is empty or cleared).
Log the results of these checks and halt further debugging if the endpoint is not healthy or the environment is misconfigured. This prevents wasted time on environment or service issues.